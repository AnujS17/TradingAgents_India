...
MAX_DATA_LOG_CHARS = 4000


def _canonical_log_args(args):
    try:
...
def _compact_log_content(message_type, content):
    if message_type != "Data" or len(content) <= MAX_DATA_LOG_CHARS:
        return content
    digest = hashlib.sha256(content.encode("utf-8", errors="replace")).hexdigest()[:12]
    omitted = len(content) - MAX_DATA_LOG_CHARS
    return (
        f"{content[:MAX_DATA_LOG_CHARS]} "
        f"... [truncated {omitted} chars; sha256={digest}; full payload retained in report files when applicable]"
    )
...
class MessageBuffer:
    ...
    def __init__(self, max_length=100):
        ...
        self._processed_tool_call_keys = set()
    ...
    def init_for_analysis(self, selected_analysts):
        ...
        self._processed_tool_call_keys.clear()
    ...
    def add_tool_call(self, tool_name, args):
        key = (tool_name, _canonical_log_args(args))
        if key in self._processed_tool_call_keys:
            return False
        self._processed_tool_call_keys.add(key)
        timestamp = datetime.datetime.now().strftime("%H:%M:%S")
...
def get_user_selections(fast: bool = False):
    ...
    console.print(
        create_question_box(
            "Step 1: Ticker Symbol",
            "Enter the exact ticker symbol to analyze, including exchange suffix when needed (examples: RELIANCE.NS, TCS.NS, HDFCBANK.BO, INFY.NS)",
            "SPY",
        )
    )
...
def get_ticker():
    ...
    ticker = questionary.text(
        ...
        or "Please enter a valid ticker symbol, e.g. RELIANCE.NS, TCS.NS, HDFCBANK.BO.",
    ).ask()
...
def run_analysis(checkpoint: bool = False, fast: bool = False, refresh: bool = False):
    ...
    # Resolve the ticker BEFORE choosing the results directory. Resolution
    # otherwise happens inside create_initial_state(), i.e. after this path is
    # already fixed: a run typed as "BLUEJET" analysed BLUEJET.NS but wrote its
    # logs to results/BLUEJET/, so the same company landed in two different
    # directories depending on how it was typed, splitting the per-ticker
    # history that memory/reflection reads back (observed on the BLUEJET run,
    # 2026-08-11). resolve_ticker_symbol is lru_cached, so the
    # create_initial_state() call below is a cache hit returning this same
    # value — no extra network round-trip, and no risk of the two disagreeing.
    #
    ...
    results_dir = Path(config["results_dir"]) / ticker_path_component / selections["analysis_date"]
    ...
    def save_message_decorator(obj, func_name):
        ...
        def wrapper(*args, **kwargs):
            ...
            content = _compact_log_content(message_type, content)
    ...
    def save_tool_call_decorator(obj, func_name):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not added:
                # MUST return the value, not a bare `return`. This wrapper
                # replaces MessageBuffer.add_tool_call on the instance, so it
                # IS add_tool_call as far as every caller is concerned. It
                # previously swallowed the boolean, so the audit loop's
                # `is_new_call = message_buffer.add_tool_call(...)` always read
                # None — and the `[Data]` payload line it gates was therefore
                # never written for ANY deterministic pre-fetch. The BLUEJET
                # run logged 26 [Tool Call] lines but only 16 [Data] lines,
                # exactly the 16 belonging to the LLM's own ReAct calls (which
                # take a different code path); news, StockTwits, Reddit, NSE
                # filings and the market snapshot recorded that they ran but
                # not what they returned, which is precisely what a
                # hallucination audit needs. Unit tests missed it because they
                # exercise MessageBuffer directly, undecorated.
                return added
    ...
    with Live(layout, refresh_per_second=4) as live:
        ...
        # Show the resolution when it changed the symbol: the whole run, every
        # report and the results directory use the resolved form, so silently
        # analysing something other than what was typed is confusing.
        ticker_note = (
            f"Selected ticker: {selections['ticker']}"
            if resolved_ticker == selections["ticker"]
            else f"Selected ticker: {selections['ticker']} (resolved to {resolved_ticker})"
        )
        message_buffer.add_message("System", ticker_note)
        ...
        # Create spinner text
        spinner_text = (
            f"Analyzing {resolved_ticker} on {selections['analysis_date']}..."
        )
        ...
        for chunk in graph.graph.stream(init_agent_state, **args):
            ...
            for audit_call in chunk.get("audit_tool_calls", []) or []:
                ...
                args = audit_call.get("args", {})
                if name:
                    # stream_mode="values" re-emits the FULL state after every
                    # node, and audit_tool_calls isn't cleared between an
                    # analyst's own chunk and the "Msg Clear <X>" node that
                    # runs right after it — so the same entries arrive again
                    # on the very next chunk. add_tool_call() already dedupes
                    # by (name, args) and returns False for a repeat; the
                    # [Data] line must be gated on that too, or the dedup is
                    # pointless — only the [Tool Call] announcement was being
                    # suppressed while the (often much larger) result kept
                    # getting re-logged every time (observed live: every
                    # analyst's full data dump duplicated once in
    ...
    if save_choice in ("Y", "YES", ""):
        ...
        default_path = Path.cwd() / "reports" / f"{ticker_path_component}_{timestamp}"
        ...
        try:
            report_file = save_report_to_disk(final_state, resolved_ticker, save_path)
