...
TICKER_INPUT_EXAMPLES = "Examples: RELIANCE.NS, TCS.NS, HDFCBANK.BO, INFY.NS"
...
def select_openrouter_model() -> str:
    ...
    preferred = [
        ("Poolside Laguna M.1 (free) - reasoning, tools, 8K output", "poolside/laguna-m.1:free"),
        ("Poolside Laguna XS.2 (free) - faster fallback", "poolside/laguna-xs.2:free"),
    ]
    seen = {mid for _, mid in preferred}
    ordered = preferred + [(name, mid) for name, mid in models if mid not in seen]

    choices = [questionary.Choice(name, value=mid) for name, mid in ordered[:8]]
