...
def test_int_coercion(monkeypatch):
    dc = _reload_with_env(
        ...
        TRADINGAGENTS_OPENROUTER_MAX_COMPLETION_TOKENS="4096",
    )
    assert dc.DEFAULT_CONFIG["max_debate_rounds"] == 3
    assert isinstance(dc.DEFAULT_CONFIG["max_debate_rounds"], int)
    assert dc.DEFAULT_CONFIG["max_risk_discuss_rounds"] == 2
    assert isinstance(dc.DEFAULT_CONFIG["max_risk_discuss_rounds"], int)
    assert dc.DEFAULT_CONFIG["openrouter_max_completion_tokens"] == 4096
    assert isinstance(dc.DEFAULT_CONFIG["openrouter_max_completion_tokens"], int)


def test_float_coercion(monkeypatch):
    dc = _reload_with_env(
        monkeypatch,
        TRADINGAGENTS_OPENROUTER_TEMPERATURE="0.1",
        TRADINGAGENTS_OPENROUTER_TOP_P="0.8",
    )
    assert dc.DEFAULT_CONFIG["openrouter_temperature"] == 0.1
    assert isinstance(dc.DEFAULT_CONFIG["openrouter_temperature"], float)
    assert dc.DEFAULT_CONFIG["openrouter_top_p"] == 0.8
    assert isinstance(dc.DEFAULT_CONFIG["openrouter_top_p"], float)
