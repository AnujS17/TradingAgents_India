...
class DataflowsConfigIsolationTests(unittest.TestCase):
    ...
    def test_partial_nested_update_preserves_existing_defaults(self):
        ...
        # Derived, not hardcoded: this test is about a partial nested update
        # preserving untouched defaults, so pinning the literal vendor chain
        # only makes it fail every time the chain is legitimately re-ordered.
        self.assertEqual(
