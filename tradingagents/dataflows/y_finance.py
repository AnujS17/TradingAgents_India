...
def _statement_currency_warning(ticker_obj) -> str:
    ...
    warning = currency_mismatch_warning(
        info.get("financialCurrency"),
        info.get("currency"),
        figures_description="raw financial statement values",
    )
    return warning + "\n\n" if warning else ""
...
def get_stock_stats_indicators_window(
    ...
    try:
        ...
        import traceback
        error_msg = f"Error getting bulk stockstats data: {e}"
        print(error_msg)
...
def _get_stock_stats_bulk(
    ...
    """
    Bulk calculation of stock stats indicators.
    Fetches data once and calculates indicator for all available dates.
    Returns dict mapping date strings to indicator values.

    Uses _prepare_ohlcv_for_wrap so dates are tracked separately from the
    StockDataFrame, avoiding the 'Date' KeyError caused by yfinance returning
    a DatetimeIndex with name=None (which reset_index() turns into 'index').
    """
    try:
        ...
        # Trigger lazy calculation for all rows at once
        _ = df[indicator]

        # Build result dict using positional alignment (int index shared by date_series and df)
        result_dict = {}
        for i, date_str in date_series.items():
            if i < len(df):
                val = normalize_indicator_value(indicator, df.iloc[i][indicator])
                result_dict[date_str] = "N/A" if pd.isna(val) else str(val)

        return result_dict
    except Exception as e:
        logger.error(f"Error in _get_stock_stats_bulk: {e}", exc_info=True)
        raise
...
def get_stockstats_indicator(
    ...
    try:
        ...
        import traceback
        error_msg = f"Error getting stockstats indicator data for indicator {indicator} on {curr_date}: {e}"
        print(error_msg)
        print(f"Traceback: {traceback.format_exc()}")
        logger.error(error_msg, exc_info=True)
        return ""
...
def get_fundamentals(
    ...
    try:
        ...
        warning = currency_mismatch_warning(info.get("financialCurrency"), info.get("currency"))
        if warning:
            header += warning + "\n\n"
